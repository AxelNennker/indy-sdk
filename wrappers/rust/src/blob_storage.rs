...
pub fn open_reader(xtype: &str, config_json: &str) -> Box<dyn Future<Output=Result<IndyHandle, IndyError>>> {
...
pub fn open_writer(xtype: &str, config_json: &str) -> Box<dyn Future<Output=Result<CommandHandle, IndyError>>> {
    let (receiver, command_handle, cb) = ClosureHandler::cb_ec_handle();
