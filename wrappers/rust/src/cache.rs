...
pub fn get_schema(pool_handle: PoolHandle,
                  ...
                  submitter_did: &str,
                  id: &str,
                  options_json: &str) -> Box<dyn Future<Output=Result<String, IndyError>>> {
...
pub fn get_cred_def(pool_handle: PoolHandle,
                    ...
                    submitter_did: &str,
                    id: &str,
                    options_json: &str) -> Box<dyn Future<Output=Rsult<String, IndyError>>> {
    let (receiver, command_handle, cb) = ClosureHandler::cb_ec_string();
...
///  }
pub fn purge_schema_cache(wallet_handle: WalletHandle, options_json: &str) -> Box<dyn Future<Output=Result<(), IndyError>>> {
...
///    maxAge: (int, optional, -1 by default) Purge cached data if older than this many seconds. -1 means purge all.
///  }
pub fn purge_cred_def_cache(wallet_handle: WalletHandle, options_json: &str) -> Box<dyn Future<Output=Result<(), IndyError>>> {
    let (receiver, command_handle, cb) = ClosureHandler::cb_ec();
